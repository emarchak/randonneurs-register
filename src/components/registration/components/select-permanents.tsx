...
const RouteDescription = ({ route }: { route: Route }) => (
  <>
    <strong>
      {route.cuesheet ? (
        <Link href={route.cuesheet}>{route.name}</Link>
      ) : (
        route.name
      )}
    </strong>
    {route.startLocation && (
      <>
        ...
        <small>
          Suggested start:{" "}
          <Link href={mapURL(route.startLocation)}>{route.startLocation}</Link>
        </small>
      </>
    )}
  </>
  ...
  const options = useMemo(() => {
    ...
    fullRoutes.forEach((route) => {
      const isChapter = filters.chapter
        ? route.chapter === filters.chapter
        : true
      const isDistance = filters.distance
        ? getIsDistance(route.distance, filters.distance)
        : true
  ...
  return (
    <>
      ...
      <RadioTable
        ...
        help={
          <>
            See the full routes in our{" "}
            <Link href="https://www.randonneursontario.ca/">route archive</Link>
            .
          </>
        }
        name="route"
