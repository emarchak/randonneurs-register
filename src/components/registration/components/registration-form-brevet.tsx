...
export const RegistrationFormBrevet = () => {
  ...
  const { onSubmit, loading, defaultScheduleId } = useRegistrationForm({
    formName,
    fieldLabels,
  })
  ...
  const handleValidStartTimes = (requestedStartTime: Date) =>
    allowedStartTimes(requestedStartTime, formData.scheduleTime || null)
  ...
  return (
    <Form name={formName} className={styles.registrationForm}>
      <ContentWrapper>
        ...
        <Aside>
          <p>
            You'll be given a brevet card at the start of each ride. Submit your
            brevet card and recorded activity (strava, ridewgps, garmin, etc.)
            to your Chapter VP when you're done.
          </p>

          <p>
            <Link href="http://randonneursontario.ca/who/whatis.html#COVID">
              Learn more about riding brevets and our COVID-19 guidelines.
            </Link>
          </p>
        </Aside>
        <SelectBrevets
        ...
        <SelectField
          ...
          help={
            <>
              The <em lang="fr">Audax Club Parisien</em> uses this for ridership
              statistics
            </>
          }
        />
        <CheckboxField
          name="shareRide"
          value={formData.shareRide}
          onChange={handleInputChange}
          optional
          help={
            <>
              Share your name with other riders before the event. All riders
              will appear on the results after the event.
            </>
          }
        >
        ...
        <HiddenField
          name="scheduleTime"
          value={formData.scheduleTime?.toString()}
        />
        ...
        <div aria-live="polite">
          ...
          {isSubmitted ? (
            <p>
              <strong>Thank you for registering to ride with us.</strong>
              <br />
              <>
                A copy of your registration request has been sent to your email,
                and the ride organizer will be in contact to confirm your
                registration.
              </>
            </p>
          ) : (
