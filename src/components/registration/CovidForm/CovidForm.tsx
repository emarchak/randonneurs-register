...
const checkScreening = (formData: FormData) =>
  [...symptomFields, ...contactFields].some((fieldName) => formData[fieldName])

const screeningResultText = (screeningStatus) =>
  screeningStatus
    ...
    const successMail = await sendMail(
      {
        ...
        data: {
          ...
          body: `<p>Here are the results of your COVID-19 screening, for your own records.</p><p><strong>${screeningResultText(
            screeningStatus
          )}</strong></p>`,
      ...
      <Form name={formName}>
        <ContentWrapper>
          ...
          <Callout alternative>
            <fieldset>
              <legend>
                <strong>1. {fieldLabels['symptoms']}</strong>
              </legend>
              <p>
                If you have an existing health condition that gives you the
                symptoms you should check these symptoms, unless the symptom is
                new, different or getting worse. Look for changes from your
                normal symptoms
              </p>
          ...
          <SubmitButton
            disabled={(hasError && !isDirty) || isSubmitted}
            loading={loading}
      ...
      <ContentWrapper>
        {isSubmitted && (
          <Callout alternative>
            ...
            <p>
              A copy has been sent to your email and has been recorded as per{' '}
              <Link href="https://ontariocycling.org/covid-19-information/">
                Ontario Cycling guidelines
              </Link>
              .
            </p>
            <p>
              <strong>{screeningResultText(screeningResult)}</strong>
            </p>
            {screeningResult && (
              <p>
                You've selected "Yes" to some of the above questions. Please
                stay home and follow regional guidelines.
              </p>
            )}
